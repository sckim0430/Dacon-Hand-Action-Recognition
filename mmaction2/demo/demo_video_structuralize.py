...
def visualize(frames,
    ...
    for i in range(na):
        ...
        for j in range(nfpa):
            ...
            frame = cv2.putText(frame, action_result, (10, 30), FONTFACE, FONTSCALE,
            ...
            for ann in anno:
                ...
                if not pose_results:
                    frame = cv2.rectangle(frame, st, ed, plate[0], 2)
                ...
                for k, lb in enumerate(label):
                    ...
                    frame = cv2.rectangle(frame, diag0, diag1, plate[k + 1], -1)
                    
                    frame = cv2.putText(frame, text, location, FONTFACE, FONTSCALE,
                                FONTCOLOR, THICKNESS, LINETYPE)
...
def rgb_based_action_recognition(args):
    ...
    label_map = [x.strip() for x in open(args.label_map).readlines()]
    rgb_action_result = label_map[action_results[0][0]]
