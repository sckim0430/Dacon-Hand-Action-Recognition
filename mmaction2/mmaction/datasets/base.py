...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def evaluate(self,
        ...
        results = [1 if value>=0.5 else 0 for value in results]
        ...
        for metric in metrics:
            if metric == 'accuracy':
                eval_results['accuracy'] = accuracy_score(results,gt_labels)
            elif metric == 'precision':
                eval_results['precision'] = precision_score(results,gt_labels)
            elif metric =='recall':
                eval_results['recall'] = recall_score(results,gt_labels)
            elif metric=='f1_score':
                eval_results['f1_score'] = f1_score(results,gt_labels)
