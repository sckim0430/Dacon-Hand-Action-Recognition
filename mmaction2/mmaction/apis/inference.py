...
def inference_recognizer(model, video, outputs=None, as_tensor=True, **kwargs):
    ...
    # print('')
    # print(data['keypoint_score'].shape)
...
def inference_recognizer_i(model, video, samples_per_gpu=1,outputs=None, as_tensor=True, **kwargs):
    """Inference a video with the recognizer.
    ...
    device = next(model.parameters()).device  # model device
    # build the data pipeline
    infer_pipeline = cfg.data.inference.pipeline
    # Alter data pipelines & prepare inputs
    if input_flag == 'dict':
        data = video
    if input_flag == 'array':
        ...
        data = dict(
            ...
            array=video,
            modality=modality)
        for i in range(len(infer_pipeline)):
            if 'Decode' in infer_pipeline[i]['type']:
                infer_pipeline[i] = dict(type='ArrayDecode')
    if input_flag == 'video':
        data = dict(filename=video, label=-1, start_index=0, modality='RGB')
        if 'Init' not in infer_pipeline[0]['type']:
            infer_pipeline = [dict(type='OpenCVInit')] + infer_pipeline
        else:
            infer_pipeline[0] = dict(type='OpenCVInit')
        for i in range(len(infer_pipeline)):
            if 'Decode' in infer_pipeline[i]['type']:
                infer_pipeline[i] = dict(type='OpenCVDecode')
    if input_flag == 'rawframes':
        filename_tmpl = cfg.data.inference.get('filename_tmpl', 'img_{:05}.jpg')
        modality = cfg.data.inference.get('modality', 'RGB')
        start_index = cfg.data.inference.get('start_index', 1)
        ...
        data = dict(
            ...
            filename_tmpl=filename_tmpl,
            modality=modality)
        if 'Init' in infer_pipeline[0]['type']:
            infer_pipeline = infer_pipeline[1:]
        for i in range(len(infer_pipeline)):
            if 'Decode' in infer_pipeline[i]['type']:
                infer_pipeline[i] = dict(type='RawFrameDecode')
    if input_flag == 'audio':
        data = dict(
            audio_path=video,
            total_frames=len(np.load(video)),
    ...
    infer_pipeline = Compose(infer_pipeline)
    data = infer_pipeline(data)
    data = collate([data], samples_per_gpu=samples_per_gpu)
