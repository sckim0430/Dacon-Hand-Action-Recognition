...
def _inference_single_pose_model(
                                ...
                                bboxes,
                                bbox_limit,
                                pose_deploy_cfg=None,
                                pose_task_processor=None,
    ...
    if pose_deploy_cfg == None or pose_task_processor==None:
        cfg = model.cfg
        ...
        input_shape = get_input_shape(pose_deploy_cfg)
 
        if len(bboxes)>bbox_limit:
            bboxes = bboxes[:bbox_limit]
        
        model_input, __ = pose_task_processor.create_input(img_or_path, bboxes.tolist(), input_shape)

        # forward the model
        with torch.no_grad():
            result = pose_task_processor.run_inference(model, model_input)[0]
...
def inference_top_down_pose_model(
    ...
    with OutputHook(model, outputs=outputs, as_tensor=False) as h:
        poses, heatmap = _inference_single_pose_model(
            ...
            pose_deploy_cfg=pose_deploy_cfg,
            pose_task_processor=pose_task_processor,
            dataset=dataset,
