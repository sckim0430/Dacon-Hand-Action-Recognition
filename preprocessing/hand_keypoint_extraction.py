...
mmpose_root = '/workspace/Dacon-Hand-Action-Recognition/mmpose'
args = abc.abstractproperty()
args.det_config = f'{mmpose_root}/demo/mmdetection_cfg/cascade_rcnn_x101_64x4d_fpn_1class.py'  # noqa: E501
args.det_checkpoint = '/home/sckim/Dataset/ckpt/pretrain_ckpt/cascade_rcnn_x101_64x4d_fpn_20e_onehand10k-dac19597_20201030.pth'  # noqa: E501
args.det_score_thr = 0.75
args.pose_config = f'{mmpose_root}/configs/hand/2d_kpt_sview_rgb_img/topdown_heatmap/onehand10k/hrnetv2_w18_onehand10k_256x256_dark.py'  # noqa: E501
args.pose_checkpoint = '/home/sckim/Dataset/ckpt/pretrain_ckpt/hrnetv2_w18_onehand10k_256x256_dark-a2f80c64_20210330.pth'  # noqa: E501
args.device = 'cuda:0'
...
train_anno = pd.read_csv(
    '/home/sckim/Dataset/Competition/dacon_hand/train.csv')
test_anno = pd.read_csv('/home/sckim/Dataset/Competition/dacon_hand/test.csv')
...
def detection_inference(args, frame_paths):
    results = []
    print('Performing Detection for each frame')
    prog_bar = mmcv.ProgressBar(len(frame_paths))
    for frame_path in frame_paths:
        result = inference_detector(detmodel, frame_path)
        # We only keep human detections with score larger than det_score_thr
        result = result[0][0][result[0][0][:, 4] >= args.det_score_thr]
...
def det_postproc(vid, det_results):
    ...
    print(f'\nHard {n_person}-hand Example, found {len(tracklets)} tracklet')
    if n_person == 1:
        if len(tracklets) == 1:
...
def pose_inference(args, frame_paths, det_results):
    ...
    print('Performing Hand Key Point Estimation for each frame')
    prog_bar = mmcv.ProgressBar(len(frame_paths))

    num_frame = len(det_results)

    hands = [len(x) for x in det_results]
    num_hand = max(hands) if len(hands) else 0
    kp = np.zeros((num_hand, num_frame, 21, 3), dtype=np.float32)
...
def hand_kp_extraction(vid, skip_postproc=False, mode='train'):
    frame_paths = extract_frame(vid)
    det_results = detection_inference(args, frame_paths)
    if not skip_postproc:
        det_results = det_postproc(vid, det_results)
    ...
    anno['frame_dir'] = os.path.splitext(osp.basename(vid))[0]

    if mode == 'train':
        anno['label'] = train_anno[train_anno['id']
                                   == anno['frame_dir']]['label'].values[0]
...
if __name__ == '__main__':
    root_dir = '/home/sckim/Dataset/Competition/dacon_hand/train'
    pkl_root_dir = '/home/sckim/Dataset/Competition/dacon_hand/train_pkl'
    ...
    for index, vid_name in tqdm(enumerate(vid_list)):
        ...
        anno = hand_kp_extraction(args.video, args.skip_postproc, mode='train')

        # Generate PKL File
        mmcv.dump(anno, args.output)

    root_dir = '/home/sckim/Dataset/Competition/dacon_hand/test'
    pkl_root_dir = '/home/sckim/Dataset/Competition/dacon_hand/test_pkl'
    ...
    for index, vid_name in tqdm(enumerate(vid_list)):
        ...
        # Get NTU Dataset Format Annotation
        anno = hand_kp_extraction(args.video, args.skip_postproc, mode='test')
